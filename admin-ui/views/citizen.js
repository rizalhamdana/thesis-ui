...
function saveCitizen() {
  isAuth();
  token = getCookie("Token");
  const birthDate = stringToDate(
    document.getElementById("input-date-of-birth").value
  );
  const data = {
  ...
  result
    .then((data) => {
      if (data instanceof Error) {
        alert("Could not insert this record, message:" + data);
    ...
    .catch((error) => {
      alert("Could not insert this record, message:" + error);
      window.location = "../citizens.html";
      return;
    });
}

...
function citizenFormMapping(data) {
  ...
  for (i = 0; i < sexOptions.length; i++) {
    console.log();
    if (data.sex === sexOptions[i].innerHTML) {
      sexOptions[i].setAttribute("selected", "true");
    }
  }

  const marriageOptions = document.getElementById("input-marriage-status")
    .children;
  for (i = 0; i < marriageOptions.length; i++) {
    if (data.married_status == marriageOptions[i].getAttribute("value")) {
